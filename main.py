...
class MultiCheckerApp(ctk.CTk):
    ...
    def _prepare_and_check(self, raw_lines, tab_name, threads, timeout, proxy, widgets):
        ...
        try:
            ...
            if tab_name in ("Crypto", "All"):
                # Strip + deduplicate all lines (no filtering вЂ” checker handles rejection)
                seen = set()
                data = []
                total = len(raw_lines)
    ...
    async def check_all(self, data, tab_name, threads, timeout, proxy, widgets):
                        ...
                        if result.get("info", {}).get("error"):
                            ...
                            if auth:
                                self.after(0, lambda a=auth: self.log_tagged(
                                    widgets, "valid", f"    в†і {i18n.t('auth_type')}: {a['auth_type']}"))
                                self.after(0, lambda a=auth: self.log_tagged(
                                    widgets, "valid", f"    в†і {i18n.t('auth_wallets')}: {a['wallets']}"))
                                self.after(0, lambda a=auth: self.log_tagged(
                                    widgets, "valid", f"    в†і {i18n.t('auth_how')}: {a['how']}"))
        ...
        # Filter results efficiently вЂ” only keep non-None results
    ...
    def show_stats(self, tab_name):
        ...
        if not self._platform_stats:
            ctk.CTkLabel(scroll, text="  вЂ”", font=("Arial", 12)).pack()

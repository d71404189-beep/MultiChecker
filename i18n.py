LANG = {
    "en": {
        "title": "Multi Checker Pro",
        "email": "Email",
        "social": "Social",
        "crypto": "Crypto",
        "games": "Games",
        "ai": "AI",
        "input_label": "Input (one per line):",
        "threads": "Threads:",
        "timeout": "Timeout:",
        "proxy": "Proxy:",
        "proxy_placeholder": "http://ip:port or file:proxy.txt",
        "start": "Start Check",
        "stop": "Stop",
        "clear": "Clear",
        "export": "Export Valid",
        "import_file": "Import File",
        "export_json": "Export JSON",
        "export_csv": "Export CSV",
        "export_txt": "Export TXT",
        "ready": "Ready",
        "no_data": "No data to check!",
        "starting": "Starting check for {} items with {} threads...",
        "completed": "Completed! Valid: {}/{}",
        "exported": "Exported to {}",
        "no_results": "No valid results to export!",
        "stopped": "Stopped!",
        "valid": "Valid",
        "invalid": "Invalid",
        "errors": "Errors",
        "total": "Total",
        "checking": "Checking...",
        "file_loaded": "Loaded {} lines from file",
        "linked": "Linked services",
        "auth_type": "Login",
        "auth_wallets": "Apps",
        "auth_how": "How to access",
        "duplicates_removed": "Removed {} duplicates",
        "filter_all": "All",
        "filter_valid": "Valid",
        "filter_invalid": "Invalid",
        "filter_errors": "Errors",
        "stats": "Statistics",
        "stats_title": "Check Statistics",
        "stats_by_platform": "By platform",
        "stats_found": "found",
        "stats_summary": "Summary",
        "stats_no_data": "No data yet. Run a check first!",
        "all_categories": "All",
        "starting_all": "Checking {} items across {} categories ({} total checks) with {} threads...",
        "preparing_data": "Preparing data (normalizing & deduplicating)...",
        "remove_duplicates": "Remove Duplicates",
        "no_data_to_dedup": "No data loaded to deduplicate!",
        "dedup_result": "Removed {} duplicates ({} unique lines remain)",
        "dedup_no_dupes": "No duplicates found ({} lines)",
        "filter_balance": "With Balance",
        "whale": "WHALE",
        "high_value": "High Value",
        "export_balance": "Export Balance",
        "no_balance_results": "No results with balance to export!",
        "exported_balance": "Exported {} accounts with balance to {}",
        "tg_token": "Telegram Bot Token",
        "tg_chat_id": "Chat ID",
        "tg_notify": "Telegram Notifications",
        "tg_enabled": "Notify on balance > $100",
        "tg_sent": "Telegram notification sent",
        "tg_error": "Telegram notification error: {}",
        "tg_summary": "✅ <b>Scan finished</b> — {0}\nChecked: {1} | Found: {2}\n💰 Total: ~${3}\n⏱ Time: {4}",
        "summary_line": "Accounts with balance: {} | Total portfolio: ~${}",
        "exchange_api_detected": "Exchange API key detected",
        "activity_active": "Active",
        "activity_low": "Low activity",
        "activity_dormant": "Dormant",
    },
    "ru": {
        "title": "Multi Checker Pro",
        "email": "Почта",
        "social": "Соцсети",
        "crypto": "Крипта",
        "games": "Игры",
        "ai": "ИИ",
        "input_label": "Ввод (по одному в строке):",
        "threads": "Потоки:",
        "timeout": "Таймаут:",
        "proxy": "Прокси:",
        "proxy_placeholder": "http://ip:port или файл:proxy.txt",
        "start": "Начать проверку",
        "stop": "Стоп",
        "clear": "Очистить",
        "export": "Экспорт валидных",
        "import_file": "Импорт файла",
        "export_json": "Экспорт JSON",
        "export_csv": "Экспорт CSV",
        "export_txt": "Экспорт TXT",
        "ready": "Готов",
        "no_data": "Нет данных для проверки!",
        "starting": "Начало проверки {} элементов в {} потоках...",
        "completed": "Завершено! Валидных: {}/{}",
        "exported": "Экспортировано в {}",
        "no_results": "Нет валидных результатов для экспорта!",
        "stopped": "Остановлено!",
        "valid": "Валидных",
        "invalid": "Невалидных",
        "errors": "Ошибок",
        "total": "Всего",
        "checking": "Проверка...",
        "file_loaded": "Загружено {} строк из файла",
        "linked": "Привязанные сервисы",
        "auth_type": "Вход",
        "auth_wallets": "Приложения",
        "auth_how": "Как зайти",
        "duplicates_removed": "Удалено {} дубликатов",
        "filter_all": "Все",
        "filter_valid": "Валидные",
        "filter_invalid": "Невалидные",
        "filter_errors": "Ошибки",
        "stats": "Статистика",
        "stats_title": "Статистика проверок",
        "stats_by_platform": "По платформам",
        "stats_found": "найдено",
        "stats_summary": "Итого",
        "stats_no_data": "Нет данных. Сначала запусти проверку!",
        "all_categories": "Все",
        "starting_all": "Проверка {} элементов по {} категориям ({} проверок) с {} потоками...",
        "preparing_data": "Подготовка данных (нормализация и дедупликация)...",
        "remove_duplicates": "Удалить дубликаты",
        "no_data_to_dedup": "Нет данных для удаления дубликатов!",
        "dedup_result": "Удалено {} дубликатов (осталось {} уникальных строк)",
        "dedup_no_dupes": "Дубликатов не найдено ({} строк)",
        "filter_balance": "С балансом",
        "whale": "КИТ",
        "high_value": "Жирный",
        "export_balance": "Экспорт с балансом",
        "no_balance_results": "Нет аккаунтов с балансом для экспорта!",
        "exported_balance": "Экспортировано {} акков с балансом в {}",
        "tg_token": "Telegram Bot Token",
        "tg_chat_id": "Chat ID",
        "tg_notify": "Telegram уведомления",
        "tg_enabled": "Уведомлять при балансе > $100",
        "tg_sent": "Telegram уведомление отправлено",
        "tg_error": "Ошибка уведомления Telegram: {}",
        "tg_summary": "✅ <b>Проверка завершена</b> — {0}\nПроверено: {1} | Найдено: {2}\n💰 Сумма: ~${3}\n⏱ Время: {4}",
        "summary_line": "Аккаунтов с балансом: {} | Общий портфель: ~${}",
        "exchange_api_detected": "Обнаружен API ключ биржи",
        "activity_active": "Активный",
        "activity_low": "Низкая активность",
        "activity_dormant": "Спящий",
    }
}

current_lang = "ru"

def t(key):
    return LANG[current_lang].get(key, key)

def set_lang(lang):
    global current_lang
    if lang in LANG:
        current_lang = lang
